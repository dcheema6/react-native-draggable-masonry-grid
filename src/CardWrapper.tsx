...
import { Animated, GestureResponderHandlers, View, ViewStyle } from 'react-native'
import { asyncWaitForMS, generateRandomInteger } from '../../../utils'
import { DraggableMasonryGridCardWrapperRef } from './types'
...
export const DraggableMasonryGridCardWrapper = forwardRef(
    (props: Props, ref: Ref<DraggableMasonryGridCardWrapperRef>) => {
        ...
        useEffect(() => {
            ...
            ;(async () => {
                await asyncWaitForMS(generateRandomInteger(0, Number.MAX_SAFE_INTEGER) % 500)
                animation.start()
            })()
            return () => {
                animation.stop()
                        ...
                        transform: [
                            ...(props.style.transform ?? []),
                            {
                                rotateX: rotationAnimation.interpolate({
                                    inputRange: [-1, 0, 1],
                                    outputRange: [`-${ROTATION_ANGLE_X_DEG}deg`, '0deg', `${ROTATION_ANGLE_X_DEG}deg`],
                                }),
                            },
                            {
                                rotateY: rotationAnimation.interpolate({
                                    inputRange: [-1, 0, 1],
                                    outputRange: [`-${ROTATION_ANGLE_Y_DEG}deg`, '0deg', `${ROTATION_ANGLE_Y_DEG}deg`],
                                }),
                            },
                            {
                                rotateZ: rotationAnimation.interpolate({
                                    inputRange: [-1, 0, 1],
                                    outputRange: [`-${ROTATION_ANGLE_Z_DEG}deg`, '0deg', `${ROTATION_ANGLE_Z_DEG}deg`],
                                }),
                            },
                        ],
                    },
